...
- $$\texttt{EM}$$ algorithm is used to fit the mixed effect model to estimate $$\mathbf{\beta}$$ and $$\mathbf{\Gamma}$$.

- Given these estimates $$\mathbf{\beta}$$ and $$\mathbf{\Gamma}$$, predictions are obtained  for the $$\mathbf{\gamma}_i$$'s using best linear unbiased prediction (BLUP)

$$
  \hat{\mathbf{\gamma}_i}= (\hat{\mathbf{\Gamma}}^{-1}/\hat{\sigma}^2+\mathbf{B}_i^T\mathbf{B}_i)^{-1}\mathbf{B}_i^T(\mathbf{Y}_i-\mathbf{B}_i\hat{\mathbf{\beta}})
$$

  Using the estimates of $$\mathbf{\beta}$$ and $$\mathbf{\Gamma}$$, we can estimate the mean and PC curves.

- Using the estimates of $$\mathbf{\beta}$$ and $$\mathbf{\Gamma}$$ and the prediction for $$\mathbf{\gamma}_i$$, we can predict the individual curve $$\mathbf{Y}_i(t)$$.
...
- Using MLE to estimate $$\mathbf{\beta}, \mathbf{\Gamma}$$
...
### Some problems with the mixed effects method
![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/2.png?raw=true)
...
$$
Y_i(t)=\mu(t)+\sum_{j=1}^k f_j(t)\alpha_{ij}+\epsilon_i(t) = \mu(t)+\mathbf{f}(t)^T\mathbf{\alpha}_i + \epsilon_i(t)
$$

subject to $$\int f_jf_l=\delta_{jl}, \ the\ Kronecker \ \delta$$, $$ \delta_{jl}=
		\begin{cases}
			1, & j=l \\
			0, & j \ne l
		\end{cases} $$
where $$\mu(t)$$ is overall mean function, $$f_j$$ is the $$j$$th PC function, $$\mathbf{f}=(f_1,\cdots,f_k)^T$$, $$\mathbf{\alpha}_i \sim (\mathbf{0},\Sigma)$$, $$\epsilon_i(t) \sim (\mathbf{0}, \sigma^2)$$ and uncorrelated
...
$$
\mu(t)=\mathbf{b}(t)^T\mathbf{\theta}_{\mu}, \ \ \mathbf{f}(t)^T=\mathbf{b}(t)^T\mathbf{\Theta} 
$$

where $$\mathbf{b}(t)$$ is a spline basis with dimension $$q$$, $$\mathbf{\Theta}$$ is $$q \times k$$ spline coefficients matrix, $$\mathbf{\theta}_{\mu}$$ is $$q$$-dimensional vector of spline coefficients

#### Restricted model

$$
Y_i(t)=\mathbf{b}(t)^T\mathbf{\theta}_{\mu}+\mathbf{b}(t)^T\mathbf{\Theta}\mathbf{\alpha}_i + \epsilon_i(t)
$$

$$
\epsilon_i(t) \sim (\mathbf{0},\sigma^2), \ \mathbf{\alpha}_i \sim (\mathbf{0}, \mathbf{D}) 
$$
...
$$
\mathbf{\Theta}^T \mathbf{\Theta}=\mathbf{I}, \ \mathbf{\epsilon}_i \sim (\mathbf{0}, \sigma^2 \mathbf{I}), \ \mathbf{\alpha}_i \sim (\mathbf{0}, \mathbf{D}) 
$$

- Parameters to estimate : $$\mathbf{\theta}_{\mu}, \mathbf{\Theta}, \mathbf{D}, \sigma^2$$
...
$$
\mathbf{\gamma}_i = [\mathbf{\Theta}, \mathbf{\Theta}^*]
						\begin{pmatrix}
							\mathbf{\alpha}_i\\
							\mathbf{\alpha}^*_i
						\end{pmatrix}
...
where $$\mathbf{\gamma}_i$$ is $$q$$-dimensional vector in mixed effect model, $$\mathbf{\Theta}^*$$ is a $$q \times (q-k)$$$ matrix, $$\mathbf{\alpha}_i$$ is a random vector of length $$q-k$$ with diagonal covariance matrix.

#### Mixed effects model

$$
\mathbf{Y}_i=\mathbf{B}_i\mathbf{\theta}_{\mu}+\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i + \mathbf{B}_i\mathbf{\Theta}^*\mathbf{\alpha}^*_i + \mathbf{\epsilon}_i 
$$

- The ruduced rank model is a submodel of the mixed effects model.
- Set $$\mathbf{\alpha}^*_i=0$$, it can be simply fitting the reduced rank model using a different algorithm.


![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/4.png?raw=true)

- The constrained mixed effects model obtained from the mixed effects model after setting the $$\mathbf{\alpha}^*_i=0$$.
...
#### Goal of FPCA
- Estimate $$\mu$$ and $$f$$
- Prediction of the basis coefficients $$\mathbf{\alpha}_i$$

Assuming the spline fit to the functions, it is equivalent to estimate $$\mathbf{\theta}_{\mu}$$, $$\mathbf{\Theta}$$ and predict the $$\mathbf{\alpha}_i$$

- Parameters to estimate : $$\mathbf{\theta}_{\mu}, \mathbf{\Theta}, \sigma^2, \mathbf{D}$$
...
### Maximum likelihood
Assume that $$\mathbf{\epsilon}_i \sim N(\mathbf{0},\sigma^2 \mathbf{I})$$ and $$\mathbf{\alpha}_i \sim N(\mathbf{0}, \mathbf{D})$$.

$$
\mathbf{Y}_i \sim N(\mathbf{B}_i\mathbf{\theta}_{\mu}, \sigma^2\mathbf{I}+\mathbf{B}_i\mathbf{\Theta}\mathbf{D}\mathbf{\Theta}^T\mathbf{B}^T_i) 
$$

The observed likelihood of the $$\mathbf{Y}_i$$'s is

$$
\prod_{i=1}^N \frac{1}{(2\pi)^{n_i/2}|\sigma^2\mathbf{I}+\mathbf{B}_i\mathbf{\Theta}\mathbf{D}\mathbf{\Theta}^T\mathbf{B}^T_i|^{1/2}} \exp \bigg \{ -\frac{1}{2}(\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu})^T(\sigma^2\mathbf{I}+\mathbf{B}_i\mathbf{\Theta}\mathbf{D}\mathbf{\Theta}^T\mathbf{B}^T_i)^{-1} (\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu}) \bigg \} 
$$

But to maximize this likelihood is a nonconvex optimization problem.

If $$\mathbf{\alpha}_i$$'s were observed, the joint likelihood is simplified to

$$
\prod_{i=1}^N \frac{1}{(2\pi)^{(n_i+k)/2}\sigma^{n_i}|\mathbf{D}|^\frac{1}{2}} \exp \bigg \{ -\frac{1}{2\sigma^2}(\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu}-\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i)^T(\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu}-\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i) - \frac{1}{2}\mathbf{\alpha}_i^T\mathbf{D}^{-1}\mathbf{\alpha}_i \bigg \} 
$$

Let $$\mathbf{\alpha}_i$$ is unobserved(missing), we can maximize this likelihood much easier using $$\texttt{EM}$$ algorithm.


### Penalized least squares
We choose $$\mathbf{\theta}_{\mu}, \mathbf{\Theta}, \mathbf{\alpha}_i$$'s to minimize the sum of squared residuals,

$$
\sum_{i=1}^N \bigg \{ (\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu}-\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i)^T(\mathbf{Y}_i-\mathbf{B}_i\mathbf{\theta}_{\mu}-\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i) + \sigma^2 \sum_{j=1}^K \frac{\alpha^2_{ij}}{\mathbf{D}_{jj}} \bigg \} 
$$
...
![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/5.png?raw=true)
...
![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/6.png?raw=true)
...
![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/7.png?raw=true)
...
- If $$\sigma^2 \rightarrow 0$$ and all curves are measured at similar time points, $$Var(\mathbf{Y}_i) \approx Var(\mathbf{\alpha}_i) = \mathbf{D}$$
- PVE $$= \frac{\mathbf{D}_{jj}}{tr(\mathbf{D})}, \ (\because \ \mathbf{D} \text{  is diagonal})$$

![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/8.png?raw=true)
...
![](https://github.com/statKim/FDA-Lab/blob/master/Principal Component Models for Sparse Functional Data/img/9.png?raw=true)
...
- Resampling the estimated $$\mathbf{\alpha}_i$$'s and residuals and generating new partial curves based on these values.
...
$$
\mathbf{X}_i = \mathbf{\theta}_{\mu}+\mathbf{\Theta}\mathbf{\alpha}_i + \mathbf{\epsilon}_i 
$$

$$
\mathbf{\epsilon}_i \sim N(\mathbf{0}, \mathbf{\Sigma}), \ \mathbf{\alpha}_i \sim N(\mathbf{0},\mathbf{D}) 
$$

where $$\mathbf{X}_i$$ are $$q$$-dimensional data vectors, $$\mathbf{\Theta}$$ is an orthogonal matrix
...
$$
\sum_{i=1}^N \Vert \mathbf{X}_i - \mathbf{\theta}_{\mu}-\mathbf{\Theta}\mathbf{\alpha}_i \Vert^2  
$$
...
$$
\mathbf{Y}_i = \mathbf{B}_i\mathbf{\theta}_{\mu}+\mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i + \mathbf{\epsilon}_i, \ Cov(\mathbf{\epsilon}_i)=\sigma^2\mathbf{I} 
$$

where $$\mathbf{\Theta}$$ is the PCs and the $$\mathbf{\alpha}_i$$'s are weights for the PC

- $$Cov(\mathbf{\epsilon}_i)=\sigma^2 \mathbf{I}$$ $$\Rightarrow$$ Solution of reduced rank model is equivalent to generalized PCA solution.
- On penalized least squares objective function,
	...
	$$
	\sum_{i=1}^N \Vert \mathbf{Y}_i - \mathbf{B}_i\mathbf{\theta}_{\mu} - \mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i \Vert^2
	$$

Let $$\hat{\mathbf{\gamma}_i}=(\mathbf{B}^T_i\mathbf{B}_i)^{-1}\mathbf{B}^T_i\mathbf{Y}_i$$, LSE of the spline coefficients for $$i$$th curve, then objective equation is

$$
\sum_{i=1}^N \Vert \mathbf{Y}_i - \mathbf{B}_i\hat{\mathbf{\gamma}_i} \Vert^2 + \sum_{i=1}^N \Vert \mathbf{B}_i\hat{\mathbf{\gamma}_i} - \mathbf{B}_i\mathbf{\theta}_{\mu} - \mathbf{B}_i\mathbf{\Theta}\mathbf{\alpha}_i \Vert^2  
$$

$$
= C(\mathbf{Y}) + \sum_{i=1}^N \Vert \hat{\mathbf{\gamma}_i} - \mathbf{\theta}_{\mu} - \mathbf{\Theta}\mathbf{\alpha}_i \Vert_{\mathbf{B}^T_i\mathbf{B}_i}^2 
$$
...
$$
\mathbf{u} \sim (\mathbf{0},\mathbf{G}), \ \mathbf{\epsilon} \sim (\mathbf{0},\mathbf{R}), \ Var(\mathbf{y})=\mathbf{V}=\mathbf{Z}\mathbf{G}\mathbf{Z}^T+\mathbf{R}, \ Cov(\mathbf{u},\mathbf{\epsilon})=\mathbf{0} 
$$

Assume $$\mathbf{y}|\mathbf{u} \sim N(\mathbf{X} \mathbf{\beta}+\mathbf{Z}\mathbf{u},\mathbf{R})$$ and $$\mathbf{u} \sim N(\mathbf{0},\mathbf{G})$$,
then likelihood function is
	
$$
-2\log f(\mathbf{y}|\mathbf{u})=N\log 2\pi+\log|\mathbf{R}|+(\mathbf{y}-\mathbf{X}  \mathbf{\beta}-\mathbf{Z}\mathbf{u})^T\mathbf{R}^{-1}(\mathbf{y}-\mathbf{X} \mathbf{\beta}-\mathbf{Z}\mathbf{u}) 
$$
...
Maximizing 
$$ l(\mathbf{\beta},\mathbf{u}|\mathbf{y})=\log f(\mathbf{y},\mathbf{u})$$ is equivalent to minimizing the sum of the two equations.

Differentiating the sum of the two equations

$$
\begin{aligned}
	\frac{\partial [-2l(\mathbf{\beta},\mathbf{u}|\mathbf{y})]}{\partial \mathbf{\beta}} &= -2\mathbf{X}^T\mathbf{R}^{-1}(\mathbf{y}-\mathbf{X}\mathbf{\beta}-\mathbf{Z}\mathbf{u})\\
	\frac{\partial [-2l(\mathbf{\beta},\mathbf{u}|\mathbf{y})]}{\partial \mathbf{u}} &= -2\mathbf{Z}^T\mathbf{R}^{-1}(\mathbf{y}-\mathbf{X}\mathbf{\beta}-\mathbf{Z}\mathbf{u})+2\mathbf{G}^{-1}\mathbf{u}
	\end{aligned} 
$$

equating them to $$0$$ gives the following system 

$$ \mathbf{X}^T\mathbf{R}^{-1}\mathbf{X}\hat{\mathbf{\beta}}+\mathbf{X}^T\mathbf{R}^{-1}\mathbf{Z}\hat{\mathbf{u}}=\mathbf{X}^T\mathbf{R}^{-1}\mathbf{y} 
$$

$$
\mathbf{Z}^T\mathbf{R}^{-1}\mathbf{X}\hat{\mathbf{\beta}}+(\mathbf{Z}^T\mathbf{R}^{-1}\mathbf{Z}+\mathbf{G}^{-1})\hat{\mathbf{u}}=\mathbf{Z}^T\mathbf{R}^{-1}\mathbf{y} 
$$
...
$$
	\begin{aligned}
	\hat{\mathbf{\beta}} &= (\mathbf{X}^T\mathbf{V}^{-1}\mathbf{X})^{-1}\mathbf{X}^T\mathbf{V}^{-1}\mathbf{y} = \hat{\mathbf{\beta}}^{GLS}\\
	\hat{\mathbf{u}} &= (\mathbf{Z}^T\mathbf{R}^{-1}\mathbf{Z}+\mathbf{G}^{-1})^{-1}\mathbf{Z}^T\mathbf{R}^{-1}(\mathbf{y}-\mathbf{X}\hat{\mathbf{\beta}})
	\end{aligned}
$$
